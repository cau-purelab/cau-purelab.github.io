...
| 2026-07-11 | Scholar 아카이브 데이터 정밀 점검 — bibtex 동일 중복 14건 제거(Research Article 접두어/Erratum/Correction 등 Scholar 중복 레코드, 원본 모두 보존), 제목-bibtex 불일치 2건 교정(적합성 피드백/사용자 질의 논문이 "로지스틱 회귀분석" 제목으로 잘못 표시되던 문제), 잘린 제목의 비논문 항목 1건 제거(Rho 485→470), MYL "Efficient visual attention"(cit. 67)에 bibtex 보강, 연도 파싱 실패 시 "0" 대신 "N/A" 표시, SHAP 논문 배지 "Under Review"→"Submitted" 변경(사용자 요청) |
