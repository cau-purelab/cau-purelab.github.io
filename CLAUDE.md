...
1. **이미지**: `public/assets/`는 Vite가 최적화하지 않고 그대로 배포됨. 추가 전 반드시 압축 — 인물 사진은 JPEG ≤800px, 래스터는 WebP ≤1600px, 파일당 200KB 이하 목표. MB급 원본 커밋 금지.
   ...
   - **주제는 AI여야 한다**: 연구 분야 일러스트는 노드·엣지·레이어·토큰 격자·결정 경계·출력 분포 같은 **AI/ML 시각 언어**로 그린다. 작업대·볼트·배관·탱크·아날로그 계기 같은 산업 설비 은유는 쓰지 않는다(2026-09-17에 한 번 그렇게 만들었다가 "AI가 아니라 기계 같다"는 지적을 받았다).
   - **도해가 아니라 작품이어야 한다**: 평평한 면과 균일한 선으로 된 교과서 그림은 거절당했다. 반투명 레이어 중첩, 다단 그라데이션, 글로우, 미세 입자 질감, 블러로 만든 심도를 쓴다. **큰 형태가 의미를 지고 작은 요소가 아름다움을 진다** — 이 구분을 지켜야 축소해도 읽히면서 풍부하다.
   - **SVG 필터는 브라우저에서 문제없다**: `feTurbulence`·`feGaussianBlur`를 써도 Chrome 렌더 시간은 필터 없는 페이지와 동일하게 측정됐다(1.7초). 다만 sharp(librsvg) 래스터라이즈는 수 분이 걸리므로, 검수 렌더가 느린 것을 성능 문제로 오해하지 말 것.
